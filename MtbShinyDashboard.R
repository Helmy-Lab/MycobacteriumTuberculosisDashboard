...
load_experiment_se <- function(file, sheet, experiment_name,
  ...
  if (length(padj_cols) > 0) {
    padj_df <- df %>% select(all_of(padj_cols))
    deg_df <- bind_cols(deg_df, padj_df)
  }
  
  if (length(sd_cols) > 0) {
    sd_df <- df %>% select(all_of(sd_cols))
  ...
  experiment_summary <- data.frame(
    ...
    pubmed_pmcid = if (!is.na(pmid) && pmid != "") {
      paste0("PMID: ", pmid)
    } else if (!is.na(pmcid) && pmcid != "") {
      paste0("PMCID: ", pmcid)
    } else {
      NA_character_
    },
    ...
    # ONE publication link
    # Priority: PubMed > PMC > DOI
    link = if (!is.na(pmid) && pmid != "") {
...
se_Poonawala <- load_experiment_se(
  file ="data/Poonawala_et_al2024_Antibiotics_transctiptome.xlsx",
  sheet = "Table S5 Control TPs (2)",
  experiment_name = "Poonawala Transcriptome",
  omics_type = "Transcriptomics",
  expression_scale = "Relative",
  doi = "10.1128/aac.01185-23",
  pmcid = "PMC11064486",
...
se_combined <- combine_se(list(se_Poonawala, se_Vilcheze, se_Shee, se_Schubert, se_Bei,se_bei2))
...
server <- function(input, output, session) {
  ...
  output$heatmap_plot <- renderPlotly({
    ...
    df_long$pubmed_pmcid <- summary_df$pubmed_pmcid[
      match(df_long$experiment, summary_df$experiment_name)
    ]
      ...
      if (!is.null(click)) {
        ...
        showModal(
          modalDialog(
            ...
            if (!is.na(pub_row$pubmed_pmcid[1])) {
              if (grepl("^PMID:", pub_row$pubmed_pmcid[1])) {
                pub_id <- sub("^PMID:\\s*", "", pub_row$pubmed_pmcid[1])
                tags$div(
                  tags$a(
                    href = paste0("https://pubmed.ncbi.nlm.nih.gov/", pub_id, "/"),
                    target = "_blank",
                    "Open PubMed Page"
                  )
                )
              } else if (grepl("^PMCID:", pub_row$pubmed_pmcid[1])) {
                pub_id <- sub("^PMCID:\\s*", "", pub_row$pubmed_pmcid[1])
                tags$div(
                  tags$a(
                    href = paste0("https://pmc.ncbi.nlm.nih.gov/articles/", pub_id, "/"),
                    target = "_blank",
                    "Open PMC Page"
                  )
                )
            ...
            if (!is.na(pub_row$doi[1]) && pub_row$doi[1] != "") {
              tags$div(
                tags$a(
                  href = paste0("https://doi.org/", pub_row$doi[1]),
                  target = "_blank",
                  "Open DOI Page"
                )
              )
  ...
  output$scatter_plot <- renderPlotly({
    ...
    # ---- Get selected transcriptomic datasets ----
    transcript_exps <- input$scatter_transcript
    transcript_cols <- rownames(colData(se_combined))[
      colData(se_combined)$experiment %in% transcript_exps
    ]
    
    # ---- Get selected proteomic datasets ----
    protein_exps <- input$scatter_protein
    protein_cols <- rownames(colData(se_combined))[
      colData(se_combined)$experiment %in% protein_exps
    ]
    
    if (length(transcript_cols) == 0 || length(protein_cols) == 0) {
      return(NULL)
    }
    
    # ---- Compute transcript values (mean across selected datasets) ----
    transcript_vals <- rowMeans(assay(se_combined)[, transcript_cols, drop = FALSE], na.rm = TRUE)
    
    # ---- Compute protein values (mean across selected datasets) ----
    protein_vals <- rowMeans(assay(se_combined)[, protein_cols, drop = FALSE], na.rm = TRUE)
    
    # ---- Convert transcript values if needed ----
    if (input$scatter_scale_x == "log2") {
      transcript_scaled <- log2(transcript_vals)
    } else {
      transcript_scaled <- transcript_vals
    }
    
    # ---- Convert protein values if needed ----
    if (input$scatter_scale_y == "log2") {
      protein_scaled <- log2(protein_vals)
    } else {
      protein_scaled <- protein_vals
    }
    ...
    df <- data.frame(
      ...
      transcript_scaled = transcript_scaled,
      protein_scaled = protein_scaled,
      selected = rownames(se_combined) %in% selected_genes,
      stringsAsFactors = FALSE
    )
    
    df$hover <- paste0(
      "<b>", df$gene_id, "</b>",
      "<br>Transcript: ", round(df$transcript, 3),
      "<br>Protein: ", round(df$protein, 3)
    )
    ...
    plot_ly() %>%
      ...
      layout(
        xaxis = list(title = paste("Transcript Abundance (", paste(transcript_exps, collapse=", "), ")", sep="")),
        yaxis = list(title = paste("Protein Abundance (", paste(protein_exps, collapse=", "), ")", sep="")),
        title = "Transcriptome vs Proteome Scatterplot"
      )
  })
